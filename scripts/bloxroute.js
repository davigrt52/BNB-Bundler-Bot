...
const Big = require("bjs-biginteger");
...
async function getOptimalGasPrice() {
  try {
    ...
    const basisPoints = Math.floor(CONFIG.GAS_PRICE_MULTIPLIER * 100);
    const adjustedGasPrice = BigInt(
      new Big(baseGasPrice.toString())
        .times(basisPoints)
        .div(100)
...
async function getFutureBlockHex() {
  ...
  const futureBlock = BigInt(
    new Big(currentBlock)
      .plus(CONFIG.FUTURE_BLOCK_OFFSET)
      .round(0, Big.roundDown)
      .toFixed(0)
  );
  console.log(`Current Block: ${currentBlock} → Target Block: ${futureBlock.toString()}`);
...
async function main() {
  ...
  try {
    ...
    const tokenSupply = BigInt(
      new Big(CONFIG.TOKEN_SUPPLY)
